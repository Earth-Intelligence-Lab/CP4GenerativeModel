...
def inference_KMeans(ys, y_hat, k_hat, qt, eps=1e-6, grid_res=100):
    ...
    if d == 1:
        ...
        volume = get_coverage_brutal(ys, means, covariances, weights, qt, k_hat, grid_res)

    return score, volume


# compute volume for a single sample through MC
def get_coverage_brutal(ys, means, covariances, weights, qt, k_hat, grid_res=100, M=None):
    d = ys.shape[1]
    if d == 1:
        buffle = (np.max(ys[:,0]) - np.min(ys[:,0])) * 0.2
        ...
        if M is None:
            M=100**d
        buffle = [(np.max(ys[:,i]) - np.min(ys[:,i])) * 0.2 for i in range(d)]
...
def summary_inference_KMeans(data_ys, data_y_hat, k_hat, qt, grid_res=100):
    """
    data: (N_test, N_ens, d)
    k_hat: scalar, number of clusters in k-NN
